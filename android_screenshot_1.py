...
# self.Poplog = subprocess.Popen(logcmd,stdout=logcat_file,stderr=subprocess.PIPE)
    ...
    def start_logcat(self,name):
        log_name = name.strip() + '.txt'
        self.logcat_file = open(log_name, 'w')
        logcmd = "adb logcat -v time"
    ...
    def stop_logcat(self):
        ...
        # subprocess.Popen("taskkill /F /T /PID " + str(self.Poplog.pid), shell=True)
...
class ScreenUI(QWidget):
    ...
    def initUI(self):
        ...
        btn_start_logcat = QPushButton("抓取日志", self)
        btn_start_logcat.resize(btn_start_logcat.sizeHint())
        btn_start_logcat.clicked.connect(self.on_click2)

        btn_stop_logcat = QPushButton("停止日志", self)
        btn_stop_logcat.resize(btn_stop_logcat.sizeHint())
        btn_stop_logcat.clicked.connect(self.on_click3)
        ...
        layout.addWidget(btn_start_logcat, 4, 0)
        layout.addWidget(btn_stop_logcat, 5, 0)
    ...
    def on_click2(self):
        name = self.edit_text.text()
        self.screen1.start_logcat(name)

    def on_click3(self):
        self.screen1.stop_logcat()
